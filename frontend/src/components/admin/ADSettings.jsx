// src/components/admin/ADSettings.jsx
...
function Alert({ type, msg }) {
  ...
  return (
    <div className={`flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold border ${
      ...
      {type === "success" ? <HiCheckCircle className="shrink-0"/> : <HiExclamationCircle className="shrink-0"/>} {msg}
    </div>
  );
}

export default function ADSettings({ token }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/users/profile`, { headers: hdrs(token) })
      .then(r => r.json())
      .then(d => { if (d.success) setPersonal({ firstName:d.data.firstName||"", lastName:d.data.lastName||"", phone:d.data.phone||"", email:d.data.email||"" }); })
      .catch(console.error)
  ...
  const savePersonal = async () => {
    ...
    try {
      ...
      setPAlert({ type:d.success?"success":"error", msg:d.success?"Profile saved.":d.message });
  ...
  const changePassword = async () => {
    ...
    if (pass.newPassword.length < 8) { setPwAlert({ type:"error", msg:"Min 8 characters." }); return; }
    ...
    <div>
      ...
      <div className="relative">
        <input type={showPw[sk] ? "text" : "password"} value={pass[fKey]}
          onChange={e => setPass(p => ({ ...p, [fKey]:e.target.value }))} className={`${inp} pr-12`}/>
  ...
  const avatar = `https://ui-avatars.com/api/?name=${personal.firstName||"A"}+${personal.lastName||"D"}&background=dbeafe&color=2563eb&bold=true&size=96`;
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <Section icon={HiUser} title="Personal Information">
        {loading ? Array.from({length:3}).map((_,i) => <div key={i}><Skeleton width={80} height={10}/><Skeleton height={44} borderRadius={12} className="mt-1.5"/></div>) : (
          <>
            <Alert type={pAlert.type} msg={pAlert.msg}/>
            <div className="flex items-center gap-4">
              <img src={avatar} alt="" className="w-16 h-16 rounded-2xl border border-gray-200"/>
              <div>
                <p className="font-black text-gray-900">{personal.firstName} {personal.lastName}</p>
                <p className="text-xs text-gray-400">{personal.email}</p>
                <span className="text-[10px] bg-blue-50 text-blue-700 font-bold px-2 py-0.5 rounded-full mt-1 inline-flex items-center gap-1">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input className={inp} value={personal.firstName} onChange={e => setPersonal(p => ({...p, firstName:e.target.value}))}/>
              </div>
              <div>
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-1.5 block">Last Name</label>
                <input className={inp} value={personal.lastName} onChange={e => setPersonal(p => ({...p, lastName:e.target.value}))}/>
              </div>
            </div>
            <div>
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-1.5 block">Email</label>
              <input className={`${inp} opacity-60 cursor-not-allowed`} value={personal.email} disabled/>
            </div>
            <div>
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-1.5 block">Phone</label>
              <input className={inp} value={personal.phone} onChange={e => setPersonal(p => ({...p, phone:e.target.value}))} placeholder="+250 7XX XXX XXX"/>
            </div>
      ...
      {/* Password */}
      <Section icon={HiLockClosed} title="Change Password">
