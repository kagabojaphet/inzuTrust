...
export default function LDAddProperty() {
  ...
  const [stepLoading, setStepLoading]   = useState(false); // skeleton between steps
  const [form, setForm] = useState({
    name: "", type: "house", rent: "", description: "",
    bedrooms: "0", bathrooms: "0",
    utilities: { water:true, electricity:true, internet:false, cleaning:false, parking:true },
    district: "Gasabo", sector: "Kacyiru", address: "",
    photos: [],
  });

  // ── Transition between steps with a brief skeleton flash ─────────────────────
  const goToStep = (next) => {
    setStepLoading(true);
    setTimeout(() => {
      setCurrentStep(next);
      setStepLoading(false);
    }, 350); // 350ms shimmer feels snappy, not slow
  };
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      const msg = error.response?.data?.message || "Validation failed. Ensure all fields are filled correctly.";
      alert(`Error: ${msg}`);
    } finally {
      setLoading(false);
    }
              ...
              return (
                <div key={step} className="relative z-10 flex flex-col items-start">
                  {stepLoading && isActive && !isComplete ? (
                    // shimmer on the currently-transitioning circle
                    ...
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-xs transition-all ${
                      ...
                      {isComplete ? <HiCheck className="text-lg" /> : stepNum}
                    </div>
                  )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-3xl border border-gray-200 p-8 min-h-[400px]">
              ...
              {/* ── Step skeletons (shown during transition) ── */}
              {stepLoading && (
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <div>
                        ...
                        <select className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3.5 text-sm outline-none focus:ring-2 focus:ring-blue-200 transition"
                          value={form.type} onChange={e => setForm({...form, type: e.target.value})}>
                          {["house","apartment","room","land","commercial"].map(t => (
                  ...
                  <div className="grid grid-cols-2 gap-6 mb-6">
                    <div>
                      ...
                      <select className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3.5 text-sm outline-none focus:ring-2 focus:ring-blue-200 transition"
                        ...
                        {["Gasabo","Nyarugenge","Kicukiro","Bugesera","Gatsibo","Kayonza","Kirehe","Ngoma","Rwamagana"].map(d => (
              ...
              {!stepLoading && currentStep === 3 && (
                <div className="animate-in fade-in slide-in-from-left-4 duration-500">
                  ...
                  <label className="border-2 border-dashed border-blue-100 rounded-3xl p-16 flex flex-col items-center justify-center bg-blue-50/20 mb-8 cursor-pointer hover:bg-blue-50/40 transition-all">
                    ...
                    <p className="text-sm font-black text-black">Click to upload property images</p>
                    <p className="text-xs text-gray-400 mt-1">PNG, JPG, WEBP — up to 6 photos</p>
                  </label>
                ...
                <div className="animate-in fade-in slide-in-from-left-4 duration-500">
                  ...
                  <div className="bg-gray-50 rounded-2xl p-6 space-y-3">
                    {[
                      ...
                      { label:"Rent",     value: form.rent ? `${Number(form.rent).toLocaleString()} RWF` : "—" },
                      { label:"Type",     value: form.type,           capitalize: true },
                      { label:"Location", value: `${form.district}, ${form.sector}` },
                      { label:"Address",  value: form.address        || "—" },
                      { label:"Photos",   value: `${form.photos.length} uploaded` },
                    ].map((row, i) => (
                      <div key={i} className="flex items-center gap-2">
                        <span className="text-xs font-black text-gray-400 uppercase w-20 shrink-0">{row.label}</span>
                        <span className={`text-sm font-semibold text-gray-800 ${row.capitalize ? "capitalize" : ""}`}>
                          {row.value}
                        </span>
                      </div>
                    ))}
            ...
            <div className="flex items-center justify-start gap-4 pt-2">
              <button onClick={prevStep} disabled={currentStep === 1 || stepLoading}
              ...
              {currentStep < 4 ? (
                <button onClick={nextStep} disabled={stepLoading}
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-3xl border border-gray-200 p-7">
              ...
              <div className="space-y-3">
                {stepLoading
                  ? Array.from({ length: 5 }).map((_, i) => <UtilitySkeleton key={i} />)
                  : Object.keys(form.utilities).map(util => (
                    <label key={util}
                      className={`flex items-center justify-between p-4 rounded-2xl border cursor-pointer transition-all ${
                        form.utilities[util] ? "bg-blue-50/40 border-blue-200" : "bg-white border-gray-100"
                      }`}>
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                          form.utilities[util] ? "bg-blue-600 text-white" : "bg-gray-50 text-gray-400"
                        }`}>
                          {utilityIcons[util]}
                        </div>
                        <span className={`text-xs font-black capitalize ${
                          form.utilities[util] ? "text-blue-900" : "text-gray-500"
                        }`}>{util}</span>
                      </div>
                      <input type="checkbox" checked={form.utilities[util]}
