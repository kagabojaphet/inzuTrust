...
import { API_BASE } from "../../config"; // Pointing to your central config

// ── Fallback mock data ───────────────────────────
const MOCK_STATS = {
  activeAgreements: 10,
  rentCollected: 4500000,
...
export default function LDOverview({ token, setActive }) {
  ...
  const occupancyPct = portfolio.total > 0 
    ? Math.round(((portfolio.occupied || 0) / portfolio.total) * 100) 
    : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5 flex justify-between items-start">
          <div>
            ...
            <p className="text-xs text-green-600 font-semibold mt-1 flex items-center gap-1">
              ↑+{stats.rentGrowth}% vs last month
            </p>
          </div>
      ...
      <div className="grid grid-cols-3 gap-5">
        <div className="col-span-2 bg-white rounded-2xl border border-gray-200 p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            >
              {["This Year", "Last Year", "Last 6 Months"].map(p => (
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={chart} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="revenue" stroke="#2563eb" strokeWidth={2.5}
                fill="url(#colorRevenue)" dot={{ r: 4, fill: "#2563eb", strokeWidth: 2, stroke: "#fff" }}
                activeDot={{ r: 6 }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        ...
        <div className="divide-y divide-gray-50">
          {tenants.map((t, i) => (
            <div key={t.id || i} className="grid grid-cols-12 gap-4 px-5 py-4 items-center hover:bg-gray-50 transition">
              <div className="col-span-3 flex items-center gap-2.5">
                <img src={`https://ui-avatars.com/api/?name=${t.name}&background=dbeafe&color=2563eb&bold=true&size=32`}
