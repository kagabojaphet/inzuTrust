...
export default function LDAgents({ token }) {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      setAgents(a);
      setProperties(p);
    } catch (e) { console.error("[LDAgents]", e.message); }
  ...
  const handleCreated = () => { setShowCreate(false); loadAll(); };
  const handleUpdated = () => { loadAll(); setViewing(null); };
    ...
    <div className="space-y-6">
      ...
      {showCreate && (
        <CreateAgentModal
          ...
          onSuccess={handleCreated}   // ← matches CreateAgentModal's prop name
        />
      )}
      {viewing && (
        <AgentDrawer
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Create and manage agents who help run your properties.</p>
        </div>
      ...
      {!loading && agents.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-200 py-20 text-center">
          ...
          <p className="text-gray-700 font-bold">No agents yet</p>
          <p className="text-sm text-gray-400 mt-1 mb-6 max-w-xs mx-auto">
            Create agent accounts to delegate property management to your team.
          </p>
          <button onClick={() => setShowCreate(true)}
