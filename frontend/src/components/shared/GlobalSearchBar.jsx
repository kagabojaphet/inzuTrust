...
// Debounced global search for all roles: tenant, landlord, admin, agent
// Usage: <GlobalSearchBar token={token} onNavigate={(tab, itemId) => setActive(tab)} />
import { useState, useEffect, useRef, useCallback } from "react";
import {
  HiSearch, HiX, HiOfficeBuilding, HiUsers, HiDocumentText,
  HiExclamationCircle, HiCog, HiUser,
} from "react-icons/hi";
import { API_BASE } from "../../config";

// ── Type → display config ─────────────────────────────────────────────────────
// HiCog replaces HiWrench (not in react-icons/hi v1)
const TYPE_META = {
  ...
  maintenance: { label: "Maintenance", icon: HiCog,              tab: "maintenance", color: "amber"  },
...
// ── Per-type title + subtitle builders ───────────────────────────────────────
function getTitle(type, item) {
  switch (type) {
    case "properties":  return item.title;
    case "tenants":
    case "users":       return `${item.firstName} ${item.lastName}`;
    case "agreements":  return item.property?.title || `Agreement #${String(item.id).slice(0,6)}`;
    case "disputes":    return item.title || item.docId || "Dispute";
...
export default function GlobalSearchBar({ token, onNavigate }) {
  ...
  const doSearch = useCallback(async (q) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setOpen(Object.keys(data.data || {}).length > 0);
      }
  ...
  const hasResults = Object.values(results).some(a => Array.isArray(a) && a.length > 0);
    ...
    <div className="relative w-full max-w-xs lg:max-w-sm" ref={boxRef}>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-9 py-2 text-sm border border-gray-100 rounded-xl bg-gray-50 outline-none focus:bg-white focus:border-blue-200 transition-all"
        />
        {loading && (
          <div className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 border-2 border-blue-200 border-t-blue-600 rounded-full animate-spin pointer-events-none"/>
        )}
        {!loading && query && (
      ...
      {/* Dropdown */}
      {open && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-2xl border border-gray-200 z-50 overflow-hidden max-h-[500px] overflow-y-auto">

          {/* Header */}
          <div className="px-4 py-2.5 border-b border-gray-100 bg-gray-50 flex items-center justify-between sticky top-0">
          ...
          {hasResults ? (
            Object.entries(results).map(([type, items]) => {
              ...
              const colorCls = COLOR[meta.color] || COLOR.blue;

              return (
                <div key={type}>
                  {/* Group label */}
                  <div className="px-4 py-2 bg-gray-50/60 border-y border-gray-100 flex items-center gap-2">
                  ...
                  {items.map(item => (
                    <button key={item.id} onClick={() => handleSelect(type, item)}
                      className="w-full text-left px-4 py-3 hover:bg-blue-50/60 transition flex items-center gap-3 border-b border-gray-50 last:border-0">
                      ...
                      {item.status && (
                        <span className="text-[9px] font-black px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 capitalize shrink-0 whitespace-nowrap">
            ...
            !loading && query.length >= 2 && (
              <div className="px-4 py-10 text-center">
                <HiSearch className="text-4xl text-gray-200 mx-auto mb-2"/>
                <p className="text-sm font-semibold text-gray-400">No results for "{query}"</p>
