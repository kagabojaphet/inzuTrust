  ...
  const handleUpload = async (files) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/disputes/${sel.id}/evidence`,
        {
          method: "POST",
          headers: {
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div>
            <div className="flex items-center gap-3 mb-1 flex-wrap">
              ...
              <span
                className={`text-xs font-black px-3 py-1 rounded-full ${ss.badge}`}
              >
                {status === "Review"
                  ? "REVIEW IN PROGRESS"
                  : status.toUpperCase()}
            ...
            <div className="flex items-center gap-4 text-xs text-gray-500 flex-wrap">
              ...
              <span className="flex items-center gap-1">
                ...
                Filed {fmtDate(sel.createdAt)}
              </span>

              {sel.deadline && (
                <span className="flex items-center gap-1 text-orange-500 font-semibold">
                  <HiExclamation className="shrink-0" />
                  Deadline: {fmtDate(sel.deadline)}
                </span>
              )}
            </div>
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex items-start gap-2 text-sm text-blue-700">
            ...
            <div>
              <p className="font-bold">
                Current Stage: {STAGES[sel.stage || 0]}
              </p>

              {sel.deadline && (
                <p className="text-xs text-blue-600 mt-0.5">
                  Both parties have until {fmtDate(sel.deadline)} to submit
                  supporting documents. InzuTrust mediators will review files
                  within 48 hours.
                </p>
              )}
            </div>
          ...
          {sel.respondent && (
            <div className="mt-3 bg-gray-50 rounded-xl p-3">
              ...
              <p className="text-sm font-bold text-gray-900">
                ...
                <span className="ml-2 text-[10px] text-gray-400 font-normal capitalize">
                  ({sel.respondent.role})
                </span>
              </p>
            </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <label
              className={`flex items-center gap-1.5 text-xs font-bold cursor-pointer transition ${
                uploading
                  ? "text-gray-400"
          ...
          {/* Upload Error */}
          {uploadError && (
            <div className="mb-3 bg-red-50 border border-red-200 text-red-600 text-xs px-3 py-2 rounded-xl">
              {uploadError}
            </div>
          )}
          ...
          {(sel.evidence || []).length === 0 ? (
            ...
            <div className="grid grid-cols-2 gap-3">
              {(sel.evidence || []).map((f, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100 hover:bg-gray-100 transition cursor-pointer"
                >
                  {/* File Icon */}
                  <div
