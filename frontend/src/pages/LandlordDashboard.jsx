...
import {
  ...
  HiCreditCard, HiCog, HiSearch, HiPlus,
...
export default function LandlordDashboard() {
  ...
  const NavItem = ({ item, mobile = false, collapsed = false }) => {
    ...
    if (item.children) {
      return (
        <div>
          <button onClick={() => collapsed ? setActive(item.children[0].id) : toggleGroup(item.id)}
            className={`w-full flex items-center ${collapsed ? "justify-center" : "gap-3"} px-4 py-3 rounded-xl text-xs font-black transition-all ${isGroupActive ? "bg-blue-50 text-blue-700" : "text-gray-500 hover:bg-gray-50"}`}>
          ...
          {!collapsed && isOpen && (
            <div className="ml-4 mt-1 space-y-0.5 border-l-2 border-gray-100 pl-3">
              {item.children.map(child => {
                ...
                return (
                  <button key={child.id} onClick={() => { setActive(child.id); if (mobile) setMobileOpen(false); }}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-all ${ca ? "bg-blue-50 text-blue-700" : "text-gray-500 hover:bg-gray-50"}`}>
    ...
    return (
      <button onClick={() => { setActive(item.id); if (mobile) setMobileOpen(false); }}
        className={`w-full flex items-center ${collapsed ? "justify-center" : "gap-3"} px-4 py-3 rounded-xl text-xs font-black transition-all ${isActive ? "bg-blue-50 text-blue-700" : "text-gray-500 hover:bg-gray-50"}`}>
    ...
    <SkeletonTheme baseColor="#f1f5f9" highlightColor="#e2e8f0">
      <div className="min-h-screen bg-[#f8f9fc] flex font-sans">
        ...
        <div className={`flex-1 flex flex-col min-h-screen transition-all duration-300 ${mainML}`}>
          ...
          <header className="bg-white/95 backdrop-blur-sm border-b border-gray-200 px-4 md:px-8 py-4 flex items-center justify-between sticky top-0 z-20 w-full">
            <div className="flex items-center gap-4">
              <button onClick={() => setMobileOpen(!isMobileOpen)} className="md:hidden p-2 text-gray-600">
                <HiMenu size={24}/>
              </button>
            ...
            <div className="flex items-center gap-2">
              ...
              {/* ── Notification Bell ── */}
              <NotificationBell token={token} onNavigate={setActive}/>
              {/* ── Profile Dropdown ── */}
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-2 py-3 flex justify-around items-center z-50">
          {[NAV[0], NAV[1], {id:"tenants",label:"Tenants",icon:HiUsers}, NAV[4], NAV[5]].map(item => (
            <button key={item.id} onClick={() => setActive(item.id)}
              className={`flex flex-col items-center gap-1 ${active===item.id?"text-blue-600":"text-gray-400"}`}>
              <item.icon size={22}/><span className="text-[10px] font-bold">{item.label.split(" ")[0]}</span>
            </button>
          ))}
