...
import {
  ...
  HiTrash, HiPlus, HiShieldCheck,
} from "react-icons/hi";
import { fmtDate, isOnline, PERM_LABELS, PERM_COLORS, apiRevoke, apiAssign } from "./agentHelpers";
...
function PermBadge({ permKey, active }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-[9px] font-black px-2 py-1 rounded-lg border transition ${
      active ? PERM_COLORS[color] : "bg-gray-50 text-gray-300 border-gray-100"
    }`}>
      {active ? "✓" : "✗"} {label}
    </span>
  );
...
export default function AgentDrawer({ agentData, token, allProperties, onClose, onUpdated }) {
  ...
  const handleRevoke = async (propertyId, propertyTitle) => {
    if (!confirm(`Remove agent from "${propertyTitle}"?`)) return;
        ...
        <div className="px-6 py-5 border-b border-gray-100 bg-gray-50 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
            <div className="relative">
              <img
                src={`https://ui-avatars.com/api/?name=${a.firstName}+${a.lastName}&background=dbeafe&color=2563eb&bold=true&size=80`}
                alt="avatar"
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              {unassigned.length > 0 && (
                ...
                >
                  <HiPlus className="text-sm" /> Assign Property
                </button>
              )}
            </div>
            ...
            {showAssign && (
              <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 mb-4 space-y-3">
                ...
                <div>
                  <p className="text-[9px] font-black text-gray-400 uppercase tracking-wider mb-2">Permissions</p>
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => { setShowAssign(false); setNewPropId(""); }}
                  ...
                  >
                    {assigning ? <div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <HiShieldCheck />}
                    Confirm Assign
                  </button>
                </div>
              </div>
            ...
            {assignedProps.length === 0 ? (
              ...
              <div className="space-y-3">
                {assignedProps.map(p => (
                  <div key={p.id} className="bg-white border border-gray-200 rounded-2xl p-4 hover:shadow-sm transition">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <button
                        onClick={() => handleRevoke(p.id, p.title)}
                        disabled={revoking === p.id}
                        className="shrink-0 p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition disabled:opacity-40"
