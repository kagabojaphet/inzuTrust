...
import {
  ...
  HiUsers, HiMenu, HiX, HiRefresh, HiExclamationCircle,
} from "react-icons/hi";
...
export default function AgentDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Derive tenants from assigned properties
      const tenantList = [];
      (pD.data || []).forEach(p => {
        if (p.property?.agreements) {
          p.property.agreements.forEach(a => {
            if (a.tenant && !tenantList.find(t => t.id === a.tenant.id)) {
              tenantList.push({ ...a.tenant, property: p.property?.title, rentAmount: a.rentAmount, status: a.status });
            }
          });
        }
      });
  ...
  const page = {
    ...
    // Agent uses the shared DisputeCenter — same component as tenant/landlord
    // userRole="agent" tells it to show the "respond" view (not the landlord's create flow)
